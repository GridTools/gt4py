...
namespace gridtools::usid {
    ...
    TEST(regression, cell2cell) {
        ...
        for (std::size_t k = 0; k < k_size; ++k) {
            EXPECT_DOUBLE_EQ(7, out_view(0, k));
            EXPECT_DOUBLE_EQ(10, out_view(1, k));
            EXPECT_DOUBLE_EQ(7, out_view(2, k));
            EXPECT_DOUBLE_EQ(10, out_view(3, k));
            EXPECT_DOUBLE_EQ(20, out_view(4, k));
            EXPECT_DOUBLE_EQ(10, out_view(5, k));
            EXPECT_DOUBLE_EQ(7, out_view(6, k));
            EXPECT_DOUBLE_EQ(10, out_view(7, k));
            EXPECT_DOUBLE_EQ(7, out_view(8, k));
        }
    }
} // namespace gridtools::usid
