...
def main():
    ...
    compilation_task = GTScriptCompilationTask(module.sten)
    generated_code = compilation_task.generate(debug=False, code_generator=code_generator)

    output_file = f"{output_dir}/generated_{module_name}_{mode}.hpp"
    ...
    if generate_icon:
        ...
        output_file = f"{output_dir}/generated_icon_{module_name}_{mode}{extension}"
        with open(output_file, "w+") as output:
