...
from gtc_unstructured.frontend.gtscript_ast import GTScriptASTNode
...
def sten(
    ...
    field_out_2: Field[[Cell, K], dtype],
    field_sparse: SparseField[[C2C, K], dtype],
):
    with computation(FORWARD), location(Cell) as c1:
        field_out[c1] = sum(field_in[c1] + field_in[c2] + field_sparse[c1, c2] for c2 in c2c[c1])
        field_out_2[c1] = sum(field_out[c1] + field_in[c2] + field_sparse[c1, c2] for c2 in c2c[c1])
