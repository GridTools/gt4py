...
class TestRegionNoExtendHigh(gt_testing.StencilTestSuite):
    ...
    def definition(field_in, field_out):
        with computation(PARALLEL), interval(...):
            from __externals__ import Jend
    ...
    def definition(field_out):
        with computation(PARALLEL), interval(...):
            ...
            with horizontal(region[:, Jstart - 1]):
                field_out = 1.0  # noqa: F841  # Local name is assigned to but never used
    ...
    def definition(inout_field, out_field):
        with computation(PARALLEL), interval(...):
            with horizontal(
                ...
                inout_field = 7.0
        with computation(PARALLEL), interval(...):
            out_field = inout_field[  # noqa: F841  # Local name is assigned to but never used
                0, -1, 0
            ]
    ...
    def definition(in_field, out_field):
        with computation(PARALLEL), interval(...):
            ...
            with horizontal(region[5, :]):
                tmp = in_field
        with computation(PARALLEL), interval(...):
            out_field = tmp[0, -1, 0]  # noqa: F841  # Local name is assigned to but never used
...
class TestRegionBoundsOutsideIterationSpaceWithExtentHigh(gt_testing.StencilTestSuite):
    ...
    def definition(in_field, inout_field, out_field):
        ...
        with computation(PARALLEL), interval(...):
            out_field = (  # noqa: F841  # Local name is assigned to but never used
                inout_field[0, -1, 0] - inout_field
            )
...
class TestRegionTwoStatementsRegionMixedDimsExtend(gt_testing.StencilTestSuite):
    ...
    def definition(in_field, out_field):
        with computation(PARALLEL), interval(...):
            ...
            with horizontal(region[0, :]):
                tmp = in_field[0, -1, 0]
                out_field = tmp  # noqa: F841  # Local name is assigned to but never used
    ...
    def definition(in_field, out_field, cond):
        with computation(PARALLEL), interval(...):
            with horizontal(region[3:-3, :]):
                if cond > 0:
                    out_field = in_field[  # noqa: F841  # Local name is assigned to but never used
                        -1, 0, 0
                    ]
        ...
        with computation(PARALLEL), interval(...):
            if cond > 0:
                with horizontal(region[3:-2, :]):
                    out_field = 7.0  # noqa: F841  # Local name is assigned to but never used
