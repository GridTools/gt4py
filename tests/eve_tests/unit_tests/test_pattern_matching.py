...
from eve.pattern_matching import ObjectPattern, get_differences, ModuleWrapper
...
register_test_case("int_unequal", 1, 2, [["a", "Objects are not equal. 1 != 2"]])
register_test_case("list_equal", [1], [1], [])
register_test_case("list_unequal", [1], [1, 2], [["a", "Expected list of length 1, but got length 2"]])
register_test_case("dict_equal", {"foo": 1}, {"foo": 1}, [])
register_test_case("dict_unequal_val", {"foo": 1}, {"foo": 2},
                   [['a["foo"]', 'Objects are not equal. 1 != 2']])
register_test_case("dict_unequal_key", {"foo": 1}, {"bar": 1},
...
register_test_case("dict_unequal_empty", {"foo": 1}, {},
                   [['a', 'Expected dictionary with keys `foo`, but the following keys are missing: `foo`']])
register_test_case("node_pattern_match", ObjectPattern(TestNode, {"bar": "baz"}), TestNode(bar="baz", foo="bar"), [])
register_test_case("node_pattern_match", ObjectPattern(TestNode, {"bar": "bar"}), TestNode(bar="baz", foo="bar"),
                   [['a.bar', 'Objects are not equal. bar != baz']])
