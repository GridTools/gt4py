...
def test_trivial(program_processor, lift_mode):
    ...
    inp_s = array_as_located_field(IDim, JDim, origin={IDim: 0, JDim: 0})(inp[:, :, 0])
    out_s = array_as_located_field(IDim, JDim)(np.zeros_like(inp[:, :, 0]))
...
def test_shifted_arg_to_lift(program_processor, lift_mode):
    ...
    inp_s = array_as_located_field(IDim, JDim, origin={IDim: 0, JDim: 0})(inp[:, :])
    out_s = array_as_located_field(IDim, JDim)(np.zeros_like(inp[:, :]))
...
def test_direct_deref(program_processor, lift_mode):
    ...
    inp_s = array_as_located_field(IDim, JDim)(inp)
    out_s = array_as_located_field(IDim, JDim)(np.zeros_like(inp))
...
def test_vertical_shift_unstructured(program_processor):
    ...
    inp_s = array_as_located_field(IDim, KDim)(inp)
    out_s = array_as_located_field(IDim, KDim)(np.zeros_like(inp))
