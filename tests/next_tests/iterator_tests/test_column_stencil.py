...
@pytest.fixture(
    params=[
        ...
        (
            ...
            lambda shape: array_as_located_field(IDim, KDim)(
        ...
        (
            ...
            lambda shape: array_as_located_field(IDim, KDim, origin={IDim: 0, KDim: 1})(
...
def test_basic_column_stencils(program_processor, lift_mode, basic_stencils):
    ...
    shape = [5, 7]
    inp = (
        array_as_located_field(IDim, KDim)(np.fromfunction(lambda i, k: i * 10 + k, shape))
        if inp_fun is None
        else inp_fun(shape)
    )
    out = array_as_located_field(IDim, KDim)(np.zeros(shape))
...
def test_ksum_scan(program_processor, lift_mode, kstart, reference):
    program_processor, validate = program_processor
    shape = [1, 7]
    inp = array_as_located_field(IDim, KDim)(np.asarray([list(range(7))]))
    out = array_as_located_field(IDim, KDim)(np.zeros(shape))
...
def test_kdoublesum_scan(program_processor, lift_mode, kstart, reference):
    ...
    shape = [1, 7]
    inp0 = array_as_located_field(IDim, KDim)(np.asarray([list(range(7))], dtype=np.float64))
    inp1 = array_as_located_field(IDim, KDim)(np.asarray([list(range(7))], dtype=np.int32))
    out = array_as_located_field(IDim, KDim)(np.zeros(shape, dtype=reference.dtype))
...
def test_different_vertical_sizes(program_processor):
    ...
    k_size = 10
    inp0 = array_as_located_field(KDim)(np.asarray(list(range(k_size))))
    inp1 = array_as_located_field(KDim)(np.asarray(list(range(k_size + 1))))
    out = array_as_located_field(KDim)(np.zeros(k_size))
    ref = inp0 + inp1[1:]
...
def test_different_vertical_sizes_with_origin(program_processor):
    ...
    k_size = 10
    inp0 = array_as_located_field(KDim)(np.asarray(list(range(k_size))))
    inp1 = array_as_located_field(KDim, origin={KDim: 1})(np.asarray(list(range(k_size + 1))))
    out = array_as_located_field(KDim)(np.zeros(k_size))
