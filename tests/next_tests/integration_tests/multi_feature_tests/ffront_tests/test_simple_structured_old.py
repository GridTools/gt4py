...
def compute_zavgS_cartesian_2(
    ...
    zavg = 0.5 * concat_where(
        IDim == domain_max_i, concat_where(
            JDim == domain_max_j,
            pp-pp,
            pp + pp(JDim + 1)),
        concat_where(JDim == domain_max_j,
...
def zavg(
    ...
    compute_zavgS_cartesian(pp, S_M, domain_max_i, domain_max_j, out=out, domain={IDim: (0, domain_max_i), JDim: (0, domain_max_j), Kolor: (0, 3)})
...
def pnabla_cartesian(
    ...
    compute_pnabla_cartesian(pp, S_M, sign, vol, max_i=domain_max_i, max_j=domain_max_j,
