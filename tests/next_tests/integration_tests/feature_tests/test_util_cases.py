...
def test_tree_mapped_np_all_close_simple_arrays_close():
    ...
    assert tree_mapped_np_all_close(a, b) is True
    assert tree_mapped_np_all_close(a, b, rtol=1e-06) is False


def test_tree_mapped_np_all_close_simple_arrays_different():
    a = np.array([1.0, 2.0, 3.0])
    b = np.array([1.1, 2.0, 3.0])
    assert tree_mapped_np_all_close(a, b) is False
...
def test_tree_mapped_np_all_close_nested_tuples():
    ...
    assert tree_mapped_np_all_close(a, c) is False
