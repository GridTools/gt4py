...
def test_offset_provider_cache(unstructured_case):
    ...
    # A second, distinct offset provider object (a subset of the case's connectivities) used to
    # check that passing a different offset provider triggers another `offset_provider_to_type`
    # call, while reusing the same object hits the cache.
    other_offset_provider = {"V2E": unstructured_case.offset_provider["V2E"]}
    ...
    with mock.patch("gt4py.next.common.offset_provider_to_type", mocked_offset_provider_to_type):
        args_1, kwargs_1 = cases.get_default_data(unstructured_case, testee)
        testee(*args_1, offset_provider=offset_provider, **kwargs_1)
        mock_offset_provider_to_type.assert_called()
        mock_offset_provider_to_type.reset_mock()

        args_2, kwargs_2 = cases.get_default_data(unstructured_case, testee)
        testee(*args_2, offset_provider=other_offset_provider, **kwargs_2)
        mock_offset_provider_to_type.assert_called()
        mock_offset_provider_to_type.reset_mock()

        args_3, kwargs_3 = cases.get_default_data(unstructured_case, testee)
        testee(*args_3, offset_provider=offset_provider, **kwargs_3)
