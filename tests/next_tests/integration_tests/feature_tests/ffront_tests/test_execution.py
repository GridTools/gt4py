...
# TODO: the test below should probably be removed as it is hard to maintain.
# Is there a way to check in the above one that something has been written to the cache after the first execution?
def test_gtfn_file_cache_extended(cartesian_case):
    if cartesian_case.backend == gtfn.run_gtfn_cached:
        cartesian_case.backend = gtfn.GTFNBackendFactory(
            gpu=False, cached=True, otf_workflow__cached_translation=True
        )
...
gridtools::meta::list<KDim_t, gridtools::integral_constant<int, 1>>>;
    ...
    parameters = (
        interface.Parameter(
            name=SymbolName("__sym_1"),
            type_=ts.FieldType(dims=[IDim, JDim, KDim], dtype=ts.ScalarType(ts.ScalarKind.INT32)),
        ),
        interface.Parameter(
            name=SymbolName("out"),
            type_=ts.FieldType(dims=[IDim, JDim, KDim], dtype=ts.ScalarType(ts.ScalarKind.INT32)),
        ...
        interface.Parameter(
            name=SymbolName("____sym_1_size_0"), type_=ts.ScalarType(ts.ScalarKind.INT32)
        ),
        interface.Parameter(
            name=SymbolName("____sym_1_size_1"), type_=ts.ScalarType(ts.ScalarKind.INT32)
        ),
        interface.Parameter(
            name=SymbolName("____sym_1_size_2"), type_=ts.ScalarType(ts.ScalarKind.INT32)
        ),
        interface.Parameter(
            name=SymbolName("__out_size_0"), type_=ts.ScalarType(ts.ScalarKind.INT32)
        ),
        interface.Parameter(
            name=SymbolName("__out_size_1"), type_=ts.ScalarType(ts.ScalarKind.INT32)
        ),
        interface.Parameter(
            name=SymbolName("__out_size_2"), type_=ts.ScalarType(ts.ScalarKind.INT32)
        ),
    )
    programm_source = stages.ProgramSource(
        entry_point=interface.Function(
            name=SymbolName("__field_operator_testee"), parameters=parameters
        ),
    ...
    with diskcache.Cache(str(config.BUILD_CACHE_DIR / "gtfn_cache")) as cache:
        ...
        assert cache[hash] == programm_source
    object.__setattr__(cartesian_case.backend.executor, "cache", {})
    cases.verify_with_default_data(cartesian_case, testee, ref=lambda a: a)
    with diskcache.Cache(str(config.BUILD_CACHE_DIR / "gtfn_cache")) as cache:
        del cache[hash]
