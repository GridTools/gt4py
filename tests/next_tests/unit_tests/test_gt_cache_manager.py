...
def write_entry(cache_base: pathlib.Path, backend: str, name: str, salt: str = "") -> str:
    ...
    file_cache = filecache.FileCache(cache.get_translation_cache_folder(cache_base, backend))
...
def write_build_dir(
    cache_base: pathlib.Path, name: str, salt: str = "0", *, version_id: str | None = None
) -> pathlib.Path:
...
def test_get_cache_base_accepts_cache_like_dirs(tmp_path, cache_base):
    ...
    cache.get_translation_cache_folder(by_translation_cache, "dace").mkdir(parents=True)
...
def test_find_entries_ignores_lock_files(cache_base):
    ...
    cache_dir = cache.get_translation_cache_folder(cache_base, "dace")
...
def test_corrupt_entry_degrades_instead_of_crashing(cache_base, capsys):
    write_entry(cache_base, "dace", "foo")
    corrupt = cache.get_translation_cache_folder(cache_base, "dace") / "deadbeefdeadbeef.pkl"
...
def test_delete_refuses_paths_outside_the_cache(cache_base, tmp_path):
    ...
    with pytest.raises(gt_cache_manager.CacheDirError, match="refusing to remove"):
        gt_cache_manager.delete_entries(
            [entry], [cache.get_translation_cache_folder(cache_base, "dace")]
        )
...
def test_delete_refuses_build_dirs_outside_the_cache(cache_base, tmp_path):
    ...
    build_dir = gt_cache_manager.BuildDir(
        program="foo", path=outsider, version_id=config.BUILD_CACHE_VERSION_ID
    )
...
def test_path_command_reports_both_caches_and_the_build_folders(cache_base, capsys):
    ...
    for backend in cache.TRANSLATION_CACHE_BACKENDS:
        assert str(cache.get_translation_cache_folder(cache_base.resolve(), backend)) in out
