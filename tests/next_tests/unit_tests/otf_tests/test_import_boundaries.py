...
- the `if TYPE_CHECKING:` branch, which never executes (its `else:` branch,
  the one a real interpreter takes, does count).
...
#: Root of the `gt4py` package, and the path its dotted module names are
#: relative to. Only the package itself is scanned: with a non-editable
#: install `SRC_ROOT` is `site-packages`, and walking that would both parse
#: every unrelated installed package and let their modules into the graph.
PACKAGE_ROOT = pathlib.Path(gt4py.__file__).resolve().parent
SRC_ROOT = PACKAGE_ROOT.parent

#: Packages that hold the GT4Py IRs and the DSL frontend.
IR_PACKAGES = ("gt4py.next.iterator", "gt4py.next.ffront")

#: Modules that must stay clear of the IRs. `otf.compilation.*` is expanded
#: to the package and all its submodules, so that a newly added file is
...
def _import_time_nodes(nodes: Iterable[ast.AST]) -> Iterator[ast.AST]:
    ...
    time, but not into function bodies or the `if TYPE_CHECKING:` branch,
    which do not. The `else:` branch of a `TYPE_CHECKING` guard *is* the
    branch a real interpreter takes, so it is descended into.
    """
    for node in nodes:
...
def _imported_names(path: pathlib.Path, module: str) -> Iterator[str]:
    ...
    # The package a relative import is resolved against: for a package
    # `__init__` that is the module's own dotted name, for a plain module its
    # parent. Getting this wrong silently drops the edge, because the
    # bogus dotted name resolves back to `module` itself.
    self_parts = module.split(".")
    if path.name != "__init__.py":
        self_parts = self_parts[:-1]
    for node in _import_time_nodes(tree.body):
        if isinstance(node, ast.Import):
            ...
            if node.level:  # relative import
                package = ".".join(self_parts[: len(self_parts) - (node.level - 1)])
...
def _expand(patterns: tuple[str, ...], graph: Mapping[str, Set[str]]) -> list[str]:
    ...
    for pattern in patterns:
        if pattern.endswith(".*"):
            # The package `__init__` is covered too: it executes on every
            # import of one of its submodules.
            package = pattern[:-2]
            prefix = f"{package}."
            matched = sorted(
                module for module in graph if module == package or module.startswith(prefix)
            )
