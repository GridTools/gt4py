...
class TestOptionDescriptorBasics:
    ...
    def test_descriptor_get_method(self) -> None:
        """Test get() method returns correct values."""

        class TestConfig(ConfigManager):
            value = OptionDescriptor(option_type=int, default=42)

        cfg = TestConfig()
        assert cfg.get("value") == 42

    def test_descriptor_rejects_unrecognized_option(self) -> None:
...
class TestConfigurationPrecedence:
    ...
    def test_context_override_takes_precedence(self) -> None:
        """Test that context overrides take precedence over global values."""
...
class TestSetMethod:
    ...
    def test_set_persists_across_accesses(self) -> None:
        """Test that set values persist across multiple accesses."""

        class TestConfig(ConfigManager):
            opt = OptionDescriptor(option_type=str, default="old")

        cfg = TestConfig()
        cfg.set("opt", "new")
        assert cfg.opt == "new"
        assert cfg.get("opt") == "new"
...
class TestContextOverrides:
    """Test ConfigManager.overrides() context manager."""

    def test_override_restores_original_value(self) -> None:
        """Test that overrides are reverted when exiting context."""
        ...
        cfg = TestConfig()
        original = cfg.opt
        with cfg.overrides(opt=999):
            pass
        assert cfg.opt == original
...
class TestEnumOptions:
    """Test configuration options with enum types."""

    def test_enum_option_with_default(self) -> None:
        """Test enum options work with default values."""
        ...
        class TestConfig(ConfigManager):
            color = OptionDescriptor(option_type=Color, default=Color.RED)

        cfg = TestConfig()
        assert cfg.color == Color.RED

    def test_enum_option_from_environment(self) -> None:
        ...
        with mock.patch.dict(os.environ, {"GT4PY_MODE": "DEBUG"}):

            class TestConfig(ConfigManager):
                mode = OptionDescriptor(option_type=Mode, default=Mode.RELEASE)
...
class TestAsDict:
    """Test ConfigManager.as_dict() method."""
    ...
    def test_as_dict_reflects_current_state(self) -> None:
        """Test that as_dict() reflects current configuration state."""
...
class TestRealConfigClass:
    """Test the actual Config class."""

    def test_config_singleton_works(self) -> None:
        """Test that the Config singleton is accessible."""
        assert isinstance(Config, type)
        cfg = Config()
        assert "debug" in cfg._option_descriptors_()

    def test_debug_option_exists(self) -> None:
        """Test that debug option exists and has correct type."""
        cfg = Config()
        assert isinstance(cfg.debug, bool)

    def test_build_cache_dir_property(self) -> None:
        """Test that build_cache_dir property works."""
        cfg = Config()
