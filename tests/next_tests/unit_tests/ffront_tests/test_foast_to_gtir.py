...
def test_fixed_len_tuple_comprehension_local_field():
    def foo(a: gtx.Field[[Edge], float64]):
        return tuple(2.0 * el for el in (a(V2E), a(V2E)))
    ...
    reference = im.let(
        iterable,
        im.make_tuple(im.as_fieldop_neighbors("V2E", "a"), im.as_fieldop_neighbors("V2E", "a")),
    )(
        im.make_tuple(
