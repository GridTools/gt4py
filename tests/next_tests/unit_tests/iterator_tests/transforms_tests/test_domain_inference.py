...
class E2VDim(common.LocalDimensionIndex): ...
...
Joff = im.cartesian_offset(JDim, JDim)
...
def run_test_program(
    ...
    actual_program = infer_domain.infer_program(testee, offset_provider=offset_provider or {})
...
def translate_domain(
    domain: itir.FunCall,
    shifts: dict[itir.CartesianOffset, int],
) -> domain_utils.SymbolicDomain:
    # the keys are already `CartesianOffset`s (self-describing offsets); only the integer
    # shift distances need to be wrapped into `OffsetLiteral`s
    shift_tuples = [(offset, im.ensure_offset(distance)) for offset, distance in shifts.items()]

    shift_list = [item for sublist in shift_tuples for item in sublist]

    # cartesian shifts are self-describing (`CartesianOffset`), so no offset provider is needed
...
def test_shift_x_y_two_inputs_literal():
    stencil = im.lambda_("arg0", "arg1")(
        im.plus(
            im.deref(im.shift(Ioff, -1)("arg0")),
            im.deref(im.shift(Joff, 1)("arg1")),
...
def test_nested_stencils():
    ...
    domain = im.domain(common.GridType.CARTESIAN, {IDim: (0, 11), JDim: (0, 7)})
    domain_inner = translate_domain(domain, {Ioff: 0, Joff: -1})
    ...
    expected_domains = {
        ...
        "in_field2": translate_domain(domain, {Ioff: 0, Joff: -2}),
    }
    actual_call, actual_domains = infer_domain.infer_expr(
...
def test_program():
    ...
    run_test_program(testee, expected)


def test_program_make_tuple():
    domain = im.domain(common.GridType.CARTESIAN, {IDim: (0, 11)})
...
def test_cond():
    ...
    domain = im.domain(common.GridType.CARTESIAN, {IDim: (0, 11)})
    domain_tmp = translate_domain(domain, {Ioff: -1})
...
def test_simple_let():
    ...
    expected_domains = {"in_field": translate_domain(domain, {Ioff: 1})}
    run_test_expr(testee, expected, domain, expected_domains)


def test_simple_let2():
...
def test_let():
    ...
    domain = im.domain(common.GridType.CARTESIAN, {IDim: (0, 11)})
    domain_a = translate_domain(domain, {Ioff: 1})
    expected = im.let(
        "a",
    ...
    expected_domains = {"in_field": translate_domain(domain, {Ioff: 2})}
    run_test_expr(testee, expected, domain, expected_domains)

    expected_domains_sym = {"in_field": translate_domain(domain, {Ioff: 2})}
...
def test_let_two_inputs():
    ...
    domain = im.domain(common.GridType.CARTESIAN, {IDim: (0, 11)})
    domain_p1 = translate_domain(domain, {Ioff: 1})
    domain_m1 = translate_domain(domain, {Ioff: -1})
    expected = im.let(
...
def test_nested_let_in_body():
    ...
    domain = im.domain(common.GridType.CARTESIAN, {IDim: (0, 11)})
    domain_p1 = translate_domain(domain, {Ioff: 1})
    domain_p2 = translate_domain(domain, {Ioff: 2})
    domain_p3 = translate_domain(domain, {Ioff: 3})
...
def test_nested_let_arg_shadowed():
    ...
    domain_p1 = translate_domain(domain, {Ioff: 1})
    domain_p3 = translate_domain(domain, {Ioff: 3})
    domain_p6 = translate_domain(domain, {Ioff: 6})
...
def test_nested_let_arg_shadowed2():
...
def test_double_nested_let_fun_expr():
    testee = im.let("inner1", premap_field(im.ref("outer", float_i_field), Ioff, 1))(
        im.let("inner2", premap_field(im.ref("inner1", float_i_field), Ioff, -1))(
...
def test_nested_let_args():
    ...
    domain = im.domain(common.GridType.CARTESIAN, {IDim: (0, 11)})
    domain_m1 = translate_domain(domain, {Ioff: -1})
    domain_m2 = translate_domain(domain, {Ioff: -2})
...
def test_tuple_get_1_make_tuple():
    testee = im.tuple_get(
        1,
        im.make_tuple(
...
def test_tuple_get_1_nested_make_tuple():
    testee = im.tuple_get(
        1,
        im.make_tuple(
...
def test_tuple_get_let_arg_make_tuple():
    testee = im.tuple_get(
        1,
...
def test_tuple_get_let_make_tuple():
    testee = im.tuple_get(
...
def test_make_tuple_2tuple_get():
    testee = im.make_tuple(
        im.tuple_get(0, im.ref("a", tuple_float_i_field)),
        im.tuple_get(1, im.ref("a", tuple_float_i_field)),
    )
...
def test_make_tuple_non_tuple_domain():
    testee = im.make_tuple(
        im.as_fieldop("deref")(im.ref("in_field1", float_i_field)),
        im.as_fieldop("deref")(im.ref("in_field2", float_i_field)),
    )
...
def test_never_accessed_domain():
    stencil = im.lambda_("arg0", "arg1")(im.deref("arg0"))
    domain = im.domain(common.GridType.CARTESIAN, {IDim: (0, 10)})
    expected_domains = {
        "in_field1": {IDim: (0, 10)},
...
def test_never_accessed_domain_tuple():
    testee = im.tuple_get(
    ...
    run_test_expr(testee, testee, domain, expected_domains)
...
def test_concat_where_two_dimensions():
...
def test_concat_where_two_dimensions_J():
    domain = im.domain(common.GridType.CARTESIAN, {IDim: (0, 20), JDim: (10, 30)})
...
def test_nested_concat_where_two_dimensions():
