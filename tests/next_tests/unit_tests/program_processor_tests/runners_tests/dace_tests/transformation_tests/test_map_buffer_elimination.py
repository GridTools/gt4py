...
def _make_war_hazard_sdfg() -> dace.SDFG:
    ...
    # The writer branch: produces the new value of `G`.
    ...
    # The reader branch: reads `G`, independent of the writer branch.
...
def test_map_buffer_elimination_war_hazard_not_applied():
    """Inlining a write into a Map that also reads the written data on an
    independent branch must be rejected (WAR hazard)."""
    sdfg = _make_war_hazard_sdfg()

    # Check the reference semantics of the unmodified SDFG.
    ref = {
        "G": np.random.rand(10),
        "A": np.random.rand(10),
        "O": np.zeros(10),
    ...
    sdfg(**args)
    assert np.allclose(args["O"], ref["G"] + 1.0)
    assert np.allclose(args["G"], 2.0 * ref["A"])
    ...
    assert count == 0, "The transformation must not inline the write into the Map."

    # The (unmodified) SDFG still computes the correct result.
    args = copy.deepcopy(ref)
    sdfg(**args)
    assert np.allclose(args["O"], ref["G"] + 1.0)
    assert np.allclose(args["G"], 2.0 * ref["A"])
