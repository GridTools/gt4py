...
def test_gtir_scan_tuple_output_single_annex_domain():
    """Tuple-output scan whose domain annex is a single (non-tuple) domain.

    Domain inference sometimes stores a single (union) domain on the `as_fieldop`
    node of a scan with tuple output. In this case, the lowering is expected to
    broadcast the domain over all tuple outputs, see `translate_scan`.
    """
    K = 20
    VAL0 = 1.2
    VAL1 = 2.1
    domain = im.get_field_domain(gtx_common.GridType.CARTESIAN, "y", [IDim, KDim])

    testee = gtir.Program(
        id="gtir_scan_tuple_output_single_annex_domain",
        function_definitions=[],
        params=[
            gtir.Sym(id="x", type=ts.FieldType(dims=[IDim, KDim], dtype=FLOAT_TYPE)),
            gtir.Sym(id="y", type=ts.FieldType(dims=[IDim, KDim], dtype=FLOAT_TYPE)),
    ...
    # Run domain inference, then overwrite the domain annex of the scan node with
    # a single (non-tuple) domain, as if it was a union of the tuple element domains.
    inferred_testee = infer_domain.infer_program(
        testee,
        offset_provider=CARTESIAN_OFFSETS,
        symbolic_domain_sizes=pass_manager._max_domain_range_sizes(CARTESIAN_OFFSETS),
    )
    scan_nodes = (
        eve.walk_values(inferred_testee)
        .if_isinstance(gtir.FunCall)
        .filter(
            lambda node: (
                cpm.is_applied_as_fieldop(node) and cpm.is_call_to(node.fun.args[0], "scan")
            )
        )
        .to_list()
    )
    assert len(scan_nodes) == 1
    assert isinstance(scan_nodes[0].annex.domain, tuple)  # sanity check on domain inference
    scan_nodes[0].annex.domain = scan_nodes[0].annex.domain[0]

    offset_provider_type = gtx_common.offset_provider_to_type(CARTESIAN_OFFSETS)
    sdfg = dace_lowering.lower_program_to_sdfg(
        inferred_testee, offset_provider_type, column_axis=KDim
    )
