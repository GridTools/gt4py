...
from gtc.numpy import npir, npir_codegen
...
def test_datatype() -> None:
    result = npir_codegen.NpirGen().visit(common.DataType.FLOAT64)
    print(result)
...
def test_literal(defined_dtype: common.DataType) -> None:
    result = npir_codegen.NpirGen().visit(npir.Literal(dtype=defined_dtype, value="42"))
    print(result)
...
def test_broadcast_literal(defined_dtype: common.DataType) -> None:
    result = npir_codegen.NpirGen().visit(
...
def test_parallel_offset() -> None:
    result = npir_codegen.NpirGen().visit(npir.AxisOffset.i(-3))
    print(result)
    assert result == "(i - 3):(I - 3)"


def test_parallel_offset_zero() -> None:
    result = npir_codegen.NpirGen().visit(npir.AxisOffset.j(0))
    print(result)
    assert result == "j:J"


def test_sequential_offset() -> None:
    result = npir_codegen.NpirGen().visit(npir.AxisOffset.k(5))
    print(result)
    assert result == "(k_ + 5):(k_ + 5 + 1)"


def test_sequential_offset_zero() -> None:
    result = npir_codegen.NpirGen().visit(npir.AxisOffset.k(0))
    print(result)
...
def test_temp_definition() -> None:
    result = npir_codegen.NpirGen().visit(VectorAssignFactory(temp_init=True, temp_name="a"))
...
def test_numerical_offset_pos() -> None:
    result = npir_codegen.NpirGen().visit(npir.NumericalOffset(value=1))
    assert result == " + 1"
...
def test_numerical_offset_zero() -> None:
    result = npir_codegen.NpirGen().visit(npir.NumericalOffset(value=0))
    assert result == ""
...
def test_horizontal_block() -> None:
    result = npir_codegen.NpirGen().visit(npir.HorizontalBlock(body=[]))
    print(result)
    match = re.match(
        r"(#.*?\n)?i, I = _di_ - 0, _dI_ \+ 0\nj, J = _dj_ - 0, _dJ_ \+ 0\n",
        result,
...
def test_vertical_pass_par() -> None:
    result = npir_codegen.NpirGen().visit(VerticalPassFactory(body=[], temp_defs=[]))
    print(result)
    match = re.match(
        (r"(#.*?\n)?" r"k, K = _dk_, _dK_\n"),
        result,
...
def test_full_computation_valid(tmp_path) -> None:
    result = npir_codegen.NpirGen.apply(
        npir.Computation(
            params=["f1", "f2", "f3", "s1"],
