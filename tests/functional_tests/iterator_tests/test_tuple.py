...
def test_field_of_extra_dim_output(backend, stencil):
    backend, validate = backend
...
def test_field_of_extra_dim_input(backend):
    backend, validate = backend
    ...
    inp1 = rng.normal(size=(shape[0], shape[1], shape[2]))
    inp2 = rng.normal(size=(shape[0], shape[1], shape[2]))
    inp = np.zeros(shape + [2])
    for i in range(shape[0]):
        for j in range(shape[1]):
            for k in range(shape[2]):
                inp[i, j, k, 0] = inp1[i, j, k]
                inp[i, j, k, 1] = inp2[i, j, k]
...
def test_field_of_2_extra_dim_input(backend):
    backend, validate = backend
    ...
    tuple_tuple_input[dom](inp, out=out, offset_provider={}, backend=backend)
    if validate:
