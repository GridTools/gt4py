...
def test_sum_edges_to_vertices(program_processor_no_gtfn_exec):
...
def test_sum_edges_to_vertices_reduce(program_processor_no_gtfn_exec):
    program_processor, validate = program_processor_no_gtfn_exec
    inp = index_field(Edge)
    out = np_as_located_field(Vertex)(np.zeros([9]))
    ref = np.asarray(list(sum(row) for row in v2e_arr))
...
def test_first_vertex_neigh_of_first_edge_neigh_of_cells_fencil(program_processor_no_gtfn_exec):
    program_processor, validate = program_processor_no_gtfn_exec
...
def test_sparse_input_field(program_processor_no_gtfn_exec):
    program_processor, validate = program_processor_no_gtfn_exec
...
def test_sparse_input_field_v2v(program_processor_no_gtfn_exec):
    program_processor, validate = program_processor_no_gtfn_exec
...
def test_slice_sparse(program_processor_no_gtfn_exec):
    program_processor, validate = program_processor_no_gtfn_exec
    inp = np_as_located_field(Vertex, V2V)(v2v_arr)
    out = np_as_located_field(Vertex)(np.zeros([9]))
...
def test_slice_twice_sparse(program_processor_no_gtfn_exec):
    program_processor, validate = program_processor_no_gtfn_exec
...
def test_shift_sliced_sparse(program_processor_no_gtfn_exec):
    program_processor, validate = program_processor_no_gtfn_exec
    inp = np_as_located_field(Vertex, V2V)(v2v_arr)
    out = np_as_located_field(Vertex)(np.zeros([9]))
...
def test_slice_shifted_sparse(program_processor_no_gtfn_exec):
    program_processor, validate = program_processor_no_gtfn_exec
    inp = np_as_located_field(Vertex, V2V)(v2v_arr)
    out = np_as_located_field(Vertex)(np.zeros([9]))
...
def test_lift(program_processor_no_gtfn_exec):
    program_processor, validate = program_processor_no_gtfn_exec
    inp = index_field(Vertex)
    out = np_as_located_field(Vertex)(np.zeros([9]))
...
def test_shift_sparse_input_field(program_processor_no_gtfn_exec):
    program_processor, validate = program_processor_no_gtfn_exec
    inp = np_as_located_field(Vertex, V2V)(v2v_arr)
    out = np_as_located_field(Vertex)(np.zeros([9]))
    ref = np.asarray(np.asarray(range(9)))
...
def test_shift_sparse_input_field2(program_processor_no_gtfn_exec):
    program_processor, validate = program_processor_no_gtfn_exec
    inp = index_field(Vertex)
...
def test_sparse_shifted_stencil_reduce(program_processor_no_gtfn_exec):
    program_processor, validate = program_processor_no_gtfn_exec
