...
def test_lazy_stencil(backend):
    ...
    def definition(field_a: gtscript.Field[np.double], field_b: gtscript.Field[np.double]):
...
def test_temporary_field_declared_in_if(backend):
    ...
    def definition(field_a: gtscript.Field[np.double]):
        with computation(PARALLEL), interval(...):
            if field_a < 0:
                field_b = -field_a
            else:
...
def test_ignore_np_errstate():
    def setup_and_run(backend, **kwargs):
        field_a = gt_storage.zeros(
            dtype=np.double, backend=backend, shape=(3, 3, 1), aligned_index=(0, 0, 0)
        )

        @gtscript.stencil(backend=backend, **kwargs)
        def divide_by_zero(field_a: gtscript.Field[np.double]):
            with computation(PARALLEL), interval(...):
                field_a = 1.0 / field_a
...
def test_stencil_without_effect(backend):
    def definition1(field_in: gtscript.Field[np.double]):
        with computation(PARALLEL), interval(...):
            tmp = 0.0

    def definition2(f_in: gtscript.Field[np.double]):
    ...
    field_in = gt_storage.ones(
        dtype=np.double, backend=backend, shape=(23, 23, 23), aligned_index=(0, 0, 0)
    )
...
def test_stage_merger_induced_interval_block_reordering(backend):
    ...
    @gtscript.stencil(backend=backend)
    def stencil(field_in: gtscript.Field[np.double], field_out: gtscript.Field[np.double]):
...
def test_lower_dimensional_inputs(backend):
    @gtscript.stencil(backend=backend)
    def stencil(
        field_3d: gtscript.Field[gtscript.IJK, np.double],
        field_2d: gtscript.Field[gtscript.IJ, np.double],
        field_1d: gtscript.Field[gtscript.K, np.double],
    ):
...
def test_lower_dimensional_masked(backend):
    @gtscript.stencil(backend=backend)
    def copy_2to3(
        cond: gtscript.Field[gtscript.IJK, np.double],
        inp: gtscript.Field[gtscript.IJ, np.double],
        outp: gtscript.Field[gtscript.IJK, np.double],
    ):
        with computation(PARALLEL), interval(...):
            if cond > 0.0:
                outp = inp
...
def test_variable_offsets(backend):
    @gtscript.stencil(backend=backend)
    def stencil_ij(
        in_field: gtscript.Field[np.double],
        out_field: gtscript.Field[np.double],
...
def test_variable_offsets_and_while_loop(backend):
    @gtscript.stencil(backend=backend)
    def stencil(
        pe1: gtscript.Field[np.double],
        pe2: gtscript.Field[np.double],
        qin: gtscript.Field[np.double],
        qout: gtscript.Field[np.double],
...
def test_nested_while_loop(backend):
    @gtscript.stencil(backend=backend)
    def stencil(field_a: gtscript.Field[np.double], field_b: gtscript.Field[np.int_]):
        with computation(PARALLEL), interval(...):
            while field_a < 1:
                add = 0
...
def test_mask_with_offset_written_in_conditional(backend):
    ...
    def stencil(outp: gtscript.Field[np.double]):
        with computation(PARALLEL), interval(...):
            cond = True
