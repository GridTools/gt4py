...
def test_table_types_rename():
    gt4py_calls = textwrap.dedent(
        """\
        from gt4py.next import common
        from gt4py.next.common import OffsetProviderType


        def types_of(args) -> OffsetProviderType:
            assert common.is_offset_provider_type(args.offset_provider_type)
            return infer(program, offset_provider_type=args.offset_provider_type)
        """
    )
    own_function = textwrap.dedent(
        """\
        def run(program, offset_provider_type): ...


        run(program, offset_provider_type=types)
        """
    )
    results, notes = _migrate(gt4py_calls=gt4py_calls, own_function=own_function)

    assert results["gt4py_calls"] == textwrap.dedent(
        """\
        from gt4py.next import common
        from gt4py.next.common import TableTypes


        def types_of(args) -> TableTypes:
            assert common.is_table_types(args.table_types)
            return infer(program, table_types=args.table_types)
        """
    )
