...
with `C2EDim`). The types of an offset provider's tables are called `table_types` now: an
`offset_provider_type=` keyword and an `.offset_provider_type` attribute are renamed, and so
are `OffsetProviderType` (now `TableTypes`) and `is_offset_provider_type`. What cannot be
rewritten from the source alone is reported instead: offset providers keyed by strings, which
become keyed by the connectivity (`{E2C: table}`), `.value` on a dimension (now `.tag`), and
`isinstance` checks against `Dimension`.
...
def _migrate_declarations(module: Module, cartesian: dict[str, str]) -> None:
    for statement, name, call, prefix, kind_of_call in _declarations(module):
        ...
        if len(target.elts) == 2:
            domain, local = (module.segment(element) for element in target.elts)
            text = (
...
#: Renamed names of `gt4py.next`, as keywords or attributes.
_RENAMED = {
    "offset_provider_type": "table_types",
    "OffsetProviderType": "TableTypes",
    "is_offset_provider_type": "is_table_types",
}
#: Those also renamed where they are imported and used unqualified; `offset_provider_type` is not,
#: since a bare name of that spelling is as likely to be a variable of the user's own.
_RENAMED_IMPORTS = {"OffsetProviderType", "is_offset_provider_type"}


def _migrate_renamed_names(module: Module) -> None:
    """
    Rename `offset_provider_type` and its relatives where they are keywords or attributes.

    A keyword is left alone, and noted, in a module that defines a function with a parameter of
    that name: the call may be to that function rather than to `gt4py.next`.
    """
    own_parameters = {
        argument.arg
    ...
    for node in ast.walk(module.tree):
        match node:
            case ast.keyword(arg=str() as name) if name in _RENAMED:
                assert node.end_lineno is not None
                if name in own_parameters:
                    module.note(node, f"'{name}=': renamed '{_RENAMED[name]}=' if it is gt4py's.")
                    continue
                rewrites.append(
                    (node.lineno - 1, node.col_offset, node.col_offset + len(name), _RENAMED[name])
                )
            case ast.Attribute(attr=name) if name in _RENAMED:
                assert node.end_lineno is not None and node.end_col_offset is not None
                start = node.end_col_offset - len(name)
                rewrites.append((node.end_lineno - 1, start, node.end_col_offset, _RENAMED[name]))
            case ast.alias(name=name) if name in _RENAMED_IMPORTS and node.asname is None:
                assert node.end_col_offset is not None
                rewrites.append(
                    (node.lineno - 1, node.col_offset, node.end_col_offset, _RENAMED[name])
                )
            case ast.Name(id=name) if name in _RENAMED_IMPORTS:
                assert node.end_col_offset is not None
                rewrites.append(
                    (node.lineno - 1, node.col_offset, node.end_col_offset, _RENAMED[name])
                )
    lines = module.lines
    for line, start, end, text in sorted(rewrites, reverse=True):
        lines[line] = lines[line][:start] + text + lines[line][end:]
    module.source = "".join(lines)
...
def migrate(sources: dict[pathlib.Path, str]) -> tuple[dict[pathlib.Path, str], list[str]]:
    ...
    for module in modules:
        ...
        rewritten = _apply(second)
        third = Module(path=module.path, source=rewritten, tree=ast.parse(rewritten))
        _migrate_renamed_names(third)
        results[module.path] = third.source
        notes += module.notes + second.notes + third.notes
