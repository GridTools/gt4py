...
def package_version(new_version: str) -> None:
    """Hardcode a new version-in-git default version, for cases when git is not available."""
    try:
        packaging.version.Version(new_version)  # Validate version string
    except packaging.InvalidVersion as e:
        rich.print(f"[red]Error:[/red] '{new_version}' is not a valid version string.")
    ...
    formatted_new_version = f"{new_version!s}+missing.version.info"
