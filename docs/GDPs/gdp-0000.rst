...
suitable for a GDP. Posting to the GT4Py `slack`_  channel is the best
way to start.

The proposal should be submitted as a draft GDP via a `GitHub pull
request`_ to the ``doc/wiki/GDPs`` directory with the name ``gdp-<n>.rst``
...
`slack`_  channel to announce the start of the discussion on the new feature.
The full GDP discussion should happen on the pull request, also including
...
GDPs are discussed on the mailing list. The possible paths of the
...
should be accepteds—see the next section for details. At this point
...
accordingly. In addition to updating the status field, at the very
...
you think a GDP is ready to be accepted, post a message to the GT4Py
slack channel with a subject line like:
...
Generally the GDP author will be the one to send this mesage, but
...
.. _`GitHub pull request`: https://github.com/GT4Py/GT4Py/pulls

.. _slack: https://gridtools.slack.com/archives/C0E0R7LQK

.. _repo: https://github.com/GT4Py/GT4Py
