...
An issue we faced when designing the iteration space feature is that the stages, where it is natural to specify the iteration space, are by the execution model specification free to be re-ordered by the runtime.
We propose adding `region()` to **either** (1) the multistages, in which case the execution order is specified by GridTool's execution model **or** (2) within a stage.
We plan to implement these in order, first finishing an implementation of (1) before attempting the second.
If we try (2) the aformentioned example becomes
...
The first line(s), or the lines ouside a `with` block, specify the default operation, and specializations are inside the blocks and appear as executed in program order.

When implemented well by a backend, such a specification would allow for regions to be specified within a stencil for example without launching another GPU kernel.
However, this is adds quite a bit of complexity to the backend implementation.
