...
GT4Py is missing such a language abstraction to express these computations, instead allowing the stencil caller to offset memory and specify custom bounds using `domain` and `origin`.
We are currently working around this by using these features, but it leads to inefficient code and switch-case like statements that call stencil functions.
