...
GT4Py treats the vertical domain specially, so top and bottom boundary conditions can be applied.
...
GT4Py is a young project, so it is understandable that such a language abstraction is not yet present.
We are currently working around it using `domain` and `origin`, but doing so leads to inefficient code that is difficult for model developers to read.
...
.. code-block:: python
    ...
    def compute_ub(uc, vc, ut, ub, dt5, dt4, grid):​
...
In the `detailed description` section below we offer a few alternatives, but all these involve adding a `region()` specifier that allows for specifying subsets of the horizontal iteration space.
Using this, the example above is transformed into:
...
This drastically reduces the complexity of the code and consolidates operations on `ub` - it is now clear what the stencil is doing to `ub` everywhere.
Model developers already undertand the idea of offsets with the stencil notation of e.g. `uc[0, -1, 0]`, which is analogous to specifying offsets from an edge of the horizontal domain.
...
We would like to specify a horizontal interval inside the stencil, similar to how a vertical interval is currently specified.
However, the fact that the horizontal domain is decomposed means that GT4Py processes will need to use different loop bounds and may need special treatment for halo regions.

As introduced above, we propose adding a new gtscript `region()` specifier that applies the stencil on a subset of the iteration space, when that iteration space has a non-zero intersection with the global region specified.
...
.. In our application we maintain a grid class with the global iteration bounds.
.. Therefore, in our case the edge objects could be created from a generic Box class as:
