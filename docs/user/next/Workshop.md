...
```
9.00 Optional welcome coffee at the CSCS kitchen HIT G21.1
9.30 Introduction
9.45 Hands-on session (additional coffee on demand)
13.00 Lunch break
14.00 Feedback session
15.45 Coffee break
16.00 Plan next workshop(s) + closing
17.00 End
```
...
def _graupel_toy_scan(state_kMinus1: tuple[float, float, float], qc_in: float, qr_in: float):
    ...
    # Local config (GT4Py does not yet support optional arguments)
