...
While this ADR was pending, the gtfn imperative backend was removed from the
codebase (#2877): `run_gtfn_imperative`, the `use_imperative_backend` flag and
the `GTFN_CPU_IMPERATIVE` test-matrix entry no longer exist, and issue #2810
was resolved by that removal. On rebasing this change, the builders therefore
drop `run_gtfn_imperative` instead of fixing it, and the two `#2810` xfails
(`test_hdiff`, `test_concat_where::test_lap_like`) go away with it — with the
imperative path gone, they marked nothing. The silent-override incident in the
Context above remains the motivating evidence for builders; only the outcome
for `run_gtfn_imperative` changed, not the decision itself.
