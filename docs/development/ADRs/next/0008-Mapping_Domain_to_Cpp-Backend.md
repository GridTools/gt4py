...
> [!NOTE]
> This ADR was written before the integration of `gt4py.storage` into `gt4py.next`, so the example is using `np_as_located_field` (since removed) instead of `gtx.as_field.partial`. The idea conveyed by the example remains unchanged.
