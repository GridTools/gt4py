...
renames break the old names outright, a one-time rotation of persistent
translation-cache keys, and a migration burden on downstream code that
subclassed the combinators.
...
- Cache keys rotate once: fingerprints embed the qualified class and field
  names, so the `ProgramWithArgs` / `definition` rename changes the keys of
  the persistent translation caches (gtfn and dace). This is subsumed by
  `BUILD_CACHE_VERSION_ID`, which already rotates them at every release, so
  there is no behavior change beyond a one-time cache miss.
