...
class SIRConverter(gt_ir.IRNodeVisitor):
    ...
    def visit_FieldDecl(self, node: gt_ir.FieldDecl, **kwargs):
        ...
        field_dimensions = sir_utils.make_field_dimensions_cartesian(
            [1 if ax in node.axes else 0 for ax in self.DOMAIN_AXES]
        )
        return sir_utils.make_field(
            name=node.name, dimensions=field_dimensions, is_temporary=False
        )
...
class BaseDawnBackend(gt_backend.BasePyExtBackend):
    ...
    @classmethod
    def generate_sources(cls, stencil_id, definition_ir, options, gt_backend_t):
...
class DawnCPUBackend(BaseDawnBackend):
    ...
    def generate_extension(cls, stencil_id, definition_ir, options, **kwargs):
        # Generate source
        gt_pyext_sources = cls.generate_sources(
        ...
        return cls.build_extension_module(
            stencil_id, gt_pyext_sources, pyext_opts, pyext_extra_include_dirs=include_dirs
        )
