...
from gt4py.ir.nodes import Assign, AxisBound, AxisInterval, BinOpExpr, BinaryOperator, BlockStmt, ComputationBlock, FieldRef, ScalarLiteral, StencilDefinition, LevelMarker, FieldDecl, IterationOrder
...
class DefIRToGTIR(IRNodeVisitor):
    ...
    GT4PY_ITERATIONORDER_TO_GTIR_LOOPORDER: ClassVar[Mapping[IterationOrder, common.LoopOrder]] = MappingProxyType(
        {
    ...
    GT4PY_LEVELMARKER_TO_GTIR_LEVELMARKER: ClassVar[Mapping[LevelMarker, common.LevelMarker]] = MappingProxyType(
        {
    ...
    GT4PY_OP_TO_GTIR_OP: ClassVar[Mapping[BinaryOperator, common.BinaryOperator]] = MappingProxyType(
        {
    ...
    def visit_StencilDefinition(self, node: StencilDefinition):
        ...
        return gtir.Computation(name=node.name, params=[self.visit(f) for f in node.api_fields], stencils=stencils)
    ...
    def visit_ComputationBlock(self, node: ComputationBlock):
        ...
        vertical_intervals = [gtir.VerticalInterval(
            horizontal_loops=horizontal_loops, start=start, end=end)]
        return gtir.Stencil(vertical_loops=[gtir.VerticalLoop(loop_order=self.GT4PY_ITERATIONORDER_TO_GTIR_LOOPORDER[node.iteration_order], vertical_intervals=vertical_intervals)])
    ...
    def visit_BinOpExpr(self, node: BinOpExpr):
        return gtir.BinaryOp(left=self.visit(node.lhs), right=self.visit(node.rhs), op=self.GT4PY_OP_TO_GTIR_OP[node.op])
    ...
    def visit_AxisBound(self, node: AxisBound):
        # TODO support VarRef
        return gtir.AxisBound(level=self.GT4PY_LEVELMARKER_TO_GTIR_LEVELMARKER[node.level], offset=node.offset)
