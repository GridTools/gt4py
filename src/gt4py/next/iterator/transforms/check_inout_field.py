...
class CheckInOutField(PreserveLocationVisitor, NodeTranslator):
    ...
    Example:
        ...
        >>> cartesian_domain = im.call("cartesian_domain")(im.call("named_range")(itir.AxisLiteral(value="IDim"), 0, 5))
        >>> ir = itir.Program(
        ...     id="test",
    ...
    def visit_SetAt(self, node: itir.SetAt, **kwargs) -> itir.SetAt:
        ...
        def check_expr(fun, args, offset_provider):
            ...
            for arg, shift in zip(args, shifts):
                for subexpr in as_fieldop_subexprs(arg):
                    if subexpr == node.target:
                        if shift not in (set(), {()}):
                            # This condition is just to filter out the trivial offsets in the horizontal and vertical.
                            if any(offset_provider[off.value].kind not in {common.DimensionKind.HORIZONTAL, common.DimensionKind.VERTICAL} or val.value != 0 for off, val in shift):
