...
#: The program folder that is generated by DaCe, see the `compiler.build_folder_mode`
#: configuration option in the DaCe documentation. If not specified the mode is
#: selected based on `DEBUG`. If `DEBUG` is enabled then the `development` mode is
#: used otherwise `production` mode is used.
DACE_FOLDER_MODE: str = os.environ.get(
    "GT4PY_DACE_FOLDER_MODE", "development" if DEBUG else "production"
).lower()
