...
from gt4py.next.program_processors.runners.dace import sdfg_callable_args
    ...
    def __sdfg_closure__(self, reevaluate: Optional[dict[str, str]] = None) -> dict[str, Any]:
        ...
        if self.connectivities:
            ...
            in_arrays_with_id = [
                ...
                if (conn_id := sdfg_callable_args.connectivity_identifier(name))
            ...
            def size_symbol_name(name: str, axis: int) -> str:
                return sdfg_callable_args.field_size_symbol_name(
                    sdfg_callable_args.connectivity_identifier(name), axis
                )
            ...
            def stride_symbol_name(name: str, axis: int) -> str:
                return sdfg_callable_args.field_stride_symbol_name(
                    sdfg_callable_args.connectivity_identifier(name), axis
                )
            ...
            for name, conn_id in in_arrays_with_id:
                if conn_id not in self.connectivity_tables_data_descriptors:
                    ...
                    self.connectivity_tables_data_descriptors[conn_id] = dace.data.Array(
                        ...
                        shape=[
                            symbols[sdfg_callable_args.field_size_symbol_name(conn_id, 0)],
                            symbols[sdfg_callable_args.field_size_symbol_name(conn_id, 1)],
                        ],
                        strides=[
                            symbols[sdfg_callable_args.field_stride_symbol_name(conn_id, 0)],
                            symbols[sdfg_callable_args.field_stride_symbol_name(conn_id, 1)],
                        ],
    ...
    for dace_parsed_arg, gt4py_program_arg in zip(
        ...
        match dace_parsed_arg:
            case dace.data.Scalar():
                assert dace_parsed_arg.dtype == sdfg_callable_args.as_dace_type(gt4py_program_arg)
            case bool() | np.bool_():
            ...
            case dace.data.Array():
                ...
                assert dace_parsed_arg.dtype == sdfg_callable_args.as_dace_type(
                    gt4py_program_arg.dtype
                )
