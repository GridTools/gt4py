...
from gt4py.next.program_processors.runners.dace import gtir_dace_utils
    ...
    def __sdfg_closure__(self, reevaluate: Optional[dict[str, str]] = None) -> dict[str, Any]:
        ...
        if self.connectivities:
            ...
            in_arrays_with_id = [
                ...
                if (conn_id := gtir_dace_utils.connectivity_identifier(name))
            ...
            def size_symbol_name(name: str, axis: int) -> str:
                return gtir_dace_utils.field_size_symbol_name(
                    gtir_dace_utils.connectivity_identifier(name), axis
                )
            ...
            def stride_symbol_name(name: str, axis: int) -> str:
                return gtir_dace_utils.field_stride_symbol_name(
                    gtir_dace_utils.connectivity_identifier(name), axis
                )
            ...
            for name, conn_id in in_arrays_with_id:
                if conn_id not in self.connectivity_tables_data_descriptors:
                    ...
                    self.connectivity_tables_data_descriptors[conn_id] = dace.data.Array(
                        ...
                        shape=[
                            symbols[gtir_dace_utils.field_size_symbol_name(conn_id, 0)],
                            symbols[gtir_dace_utils.field_size_symbol_name(conn_id, 1)],
                        ],
                        strides=[
                            symbols[gtir_dace_utils.field_stride_symbol_name(conn_id, 0)],
                            symbols[gtir_dace_utils.field_stride_symbol_name(conn_id, 1)],
                        ],
    ...
    for dace_parsed_arg, gt4py_program_arg in zip(
        ...
        match dace_parsed_arg:
            case dace.data.Scalar():
                assert dace_parsed_arg.dtype == gtir_dace_utils.as_dace_type(gt4py_program_arg)
            case bool() | np.bool_():
            ...
            case dace.data.Array():
                ...
                assert dace_parsed_arg.dtype == gtir_dace_utils.as_dace_type(
