...
class GTIRToSDFG(eve.NodeVisitor, SDFGBuilder):
    ...
    def visit_Lambda(
        ...
        * arguments (scalars only) that can be evaluated as symbolic expressions,
        ...
        # Note that we only consider global access nodes, because the goal of this
        # cleanup is to remove isolated nodes. At this stage, temporary data nodes
        # are supposed to contain the result of some argument expression.
