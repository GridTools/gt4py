...
class FieldopData:
    ...
    def map_to_parent_sdfg(
        ...
        inner_ctx: gtir_sdfg.InStateContext,
        outer_ctx: gtir_sdfg.InStateContext,
        ...
        """
        inner_desc = inner_ctx.desc(self.dc_node)
        assert inner_desc.transient
        inner_desc.transient = False

        if isinstance(self.gt_type, ts.ScalarType):
            outer, outer_desc = sdfg_builder.add_temp_scalar(outer_ctx, inner_desc.dtype)
            outer_origin = []
        else:
            outer, outer_desc = sdfg_builder.add_temp_array_like(outer_ctx, inner_desc)
        ...
        outer_node = outer_ctx.add_access(outer)
    ...
    def get_local_view(
        ...
        if isinstance(self.gt_type, ts.ScalarType):
            ...
            return gtir_dataflow.MemletExpr(
                ...
                subset=dace_sbs.Range.from_string("0"),
            )
...
def _create_field_operator_impl(
    ...
    if isinstance(output_edge.result.gt_dtype, ts.ScalarType):
        ...
        field_subset = field_subset + dace_sbs.Range.from_array(dataflow_output_desc)

    # allocate local temporary storage
    if len(field_shape) == 0:  # zero-dimensional field
        field_name, _ = sdfg_builder.add_temp_scalar(ctx, dataflow_output_desc.dtype)
        field_subset = dace_sbs.Range.from_string("0")
    else:
        field_name, _ = sdfg_builder.add_temp_array(ctx, field_shape, dataflow_output_desc.dtype)
...
def _create_field_operator(
    ...
    map_entry, map_exit = sdfg_builder.add_map(ctx, "fieldop", map_range)
...
def translate_as_fieldop(
    ...
    input_edges, output_edges = gtir_dataflow.translate_lambda_to_dataflow(
        ctx, sdfg_builder, stencil_expr, fieldop_args
    )
...
def _construct_if_branch_output(
    ...
    if isinstance(sym.type, ts.ScalarType):
        ...
        out, _ = sdfg_builder.add_temp_scalar(ctx, dtype)
        out_node = ctx.add_access(out)
    ...
    out, _ = sdfg_builder.add_temp_array(ctx, shape, dtype)
    out_node = ctx.add_access(out)
...
def _write_if_branch_output(
    ...
    dst_node = ctx.add_access(dst.dc_node.data)
    dst_shape = ctx.desc(dst_node).shape
    ...
    if isinstance(src.gt_type, ts.ScalarType):
        ...
        ctx.state.add_nedge(
            ...
            dace.Memlet(
                ...
                subset=data_subset,
                other_subset=dace_sbs.Range.from_array(ctx.desc(dst_node)),
            ),
        )
...
def translate_if(
    ...
    ctx.sdfg.add_edge(cond_state, true_state, dace.InterstateEdge(condition=f"{if_stmt}"))
    ...
    true_br_ctx = gtir_sdfg.InStateContext(sdfg=ctx.sdfg, state=true_state)
    true_br_result = sdfg_builder.visit(true_expr, ctx=true_br_ctx)
    false_br_ctx = gtir_sdfg.InStateContext(sdfg=ctx.sdfg, state=false_state)
    ...
    if isinstance(node.type, ts.TupleType):
        ...
        gtx_utils.tree_map(
            lambda src, dst, ctx=true_br_ctx: _write_if_branch_output(ctx, src, dst)
        )(true_br_result, node_output)
        gtx_utils.tree_map(
            lambda src, dst, ctx=false_br_ctx: _write_if_branch_output(ctx, src, dst)
        ...
        _write_if_branch_output(true_br_ctx, true_br_result, node_output)
        _write_if_branch_output(false_br_ctx, false_br_result, node_output)
...
def translate_index(
    ...
    index_data, _ = sdfg_builder.add_temp_scalar(ctx, INDEX_DTYPE)
    index_node = ctx.add_access(index_data)
...
def _get_data_nodes(
    ...
    if isinstance(data_type, ts.FieldType):
        data_node = ctx.add_access(data_name)
        return sdfg_builder.make_field(data_node, data_type)

    elif isinstance(data_type, ts.ScalarType):
        if data_name in ctx.sdfg.symbols:
            data_node = _get_symbolic_value(
                ctx, sdfg_builder, data_name, data_type, temp_name=f"__{data_name}"
            )
        else:
            data_node = ctx.add_access(data_name)
        ...
        return gtx_utils.tree_map(lambda sym: _get_data_nodes(ctx, sdfg_builder, sym.id, sym.type))(
            symbol_tree
        )
...
def _get_symbolic_value(
    ...
    data_node = ctx.add_access(temp_name)  # type: ignore[arg-type]
    ctx.state.add_edge(
        tasklet_node,
        "__out",
        data_node,
...
def translate_literal(
    ...
    data_type = node.type
    data_node = _get_symbolic_value(ctx, sdfg_builder, node.value, data_type)
...
def translate_scalar_expr(
    ...
    temp_name, _ = sdfg_builder.add_temp_scalar(ctx, gtx_dace_utils.as_dace_type(node.type))
...
def translate_symbol_ref(
    ...
    return _get_data_nodes(ctx, sdfg_builder, symbol_name, gt_symbol_type)
