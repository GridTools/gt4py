...
class ScalarBrodcastInliner(dace_transformation.SingleStateTransformation):
    """

    The transformation performs two kind of operations that are related to
    broadcast library nodes. However, this transformations only operate on
    scalar values that are broadcasted.

    The first one is "static access replacement" in that operation the pattern:
    ```
    ...
    ```
        (value_to_broadcast) -> (bcast_result) -> (access_node_consumer)
    ```
    to:
    ```
        (value_to_broadcast) -> (access_node_consumer)
    ```
    ...
    Meaning `bcast_result` can have multiple consumer.
