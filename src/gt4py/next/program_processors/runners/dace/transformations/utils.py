...
_unique_name_counter = itertools.count()


def unique_name(name: str) -> str:
    """Adds a unique string to `name`.

    Uses a deterministic counter instead of a UUID to ensure stable,
    reproducible names across consecutive compilations.

    Note:
        The names generated by this function are rather unstable and it should
        not be used if a particular order should be enforced. This function is
        marked for deprecation.
    """
    maximal_length = 200
    unique_suffix = str(next(_unique_name_counter))
    if len(name) > (maximal_length - len(unique_suffix)):
        name = name[: (maximal_length - len(unique_suffix) - 1)]
