...
def order_nodes(
    ...
    The result should be considered as the best way to order a group of nodes inside
    a state. It does, however, not guarantees a stable order in any way.

    The condition this function works best is if the nodes have a unique labels
    and AccessNodes referring to the same data have different degrees (one node used
    for reading one for writing).

    Known pathological cases:
    - Multiple top level AccessNodes referring to the same data that have the same
        degree (uncommon in GT4Py).
    """
    ...
    def key_fun(node: dace_nodes.Node) -> tuple[str, str, int, int]:
        if isinstance(node, dace_nodes.AccessNode):
            ...
            # TODO(phimuell): Maybe add `node.code` in case of Tasklets?
            nid = node.label
        else:
            nid = str(node)

        return (type(node).__name__, nid, state.in_degree(node), state.out_degree(node))

    return sorted(nodes, key=key_fun)
...
def order_edges(
    ...
    There is no guarantee of a stable order, although it should be consider more stable
    than the one generated by `order_nodes()`. However, the order might depends on the
    selected specialization level. Similar to `order_nodes()` this function works best
    ...
    #  tie breaker the subsets are used. This means that the specialization level
    #  is also involved because the subset `[a, b]` is different from `[1, 10]`.
