...
def gt_replace_concat_where_node(
    ...
    genuine_consumer_specs, descening_points = find_consumer_result
    ...
    producer_specs = _map_data_into_nested_scopes(
        ...
        consumer_specs=genuine_consumer_specs + descening_points,
    )
...
class _ProducerSpec:
    ...
    Args:
        ...
        needed_symbols: Contains the type of every free symbol of the data descriptor.
...
def _setup_initial_producer_description_on_top_level(
    ...
    for converging_edge in converging_edges:
        ...
        needed_symbols = {str(fs): sdfg_symbols[str(fs)] for fs in producer_desc.free_symbols}
...
def _setup_initial_producer_description_in_nested_state(
    ...
    assert False, "Rework this funcrtion."

    # TODO: Probably this function should only set up the `nested_base_sources` list, as the other
    #   data is generated by `_configure_descending_point()` but It should still create the data
    #   this also means that it should do the adding of symbols.
    ...
    for tl_prod_spec in initial_producer_specs:
        ...
        if nested_data_name not in nested_sdfg.arrays:
            assert False, "Has been moved to other location."
        ...
        assert len(tl_prod_spec.needed_symbols) == 0

        # Rename this variable, the name is better for `_configure_descending_point()`, but the name
        #  of the function is not so good.
        nested_initial_producer_specs.append(
            _ProducerSpec(
                ...
                desc=tl_prod_spec.desc.clone(),
                needed_symbols=tl_prod_spec.needed_symbols.copy(),
                data_source={},
            )
        )
...
def _configure_descending_point(
    top_state: dace.SDFGState,
    descening_point_spec: _FinalConsumerSpec,
    parent_initial_producer_specs: Sequence[_ProducerSpec],
    full_parent_priducer_specs: Sequence[_ProducerSpec],
    ...
    The function returns the name of the concat where data inside the nested SDFG
    and the initial producer specification, that can be passed to TBA.

    Args:
        top_state: The state containing the descending point.
        descening_point_spec: The consumer that represents a descending point.
        parent_initial_producer_specs: The initial producer specification on the top level.
        full_parent_priducer_specs: The fully processed producer specification of the parent.
    """
    assert False, "Per nested SDFG this function should only be called once."
    assert isinstance(descening_point_spec.consumer, dace_nodes.NestedSDFG)
    nsdfg_node: dace_nodes.NestedSDFG = descening_point_spec.consumer
    ...
    top_concat_data = descening_point_spec.edge.data.data
    ...
    nested_to_top_desc_mapping = gtx_transformations.utils.gt_data_descriptor_mapping(
        top_state, nsdfg_node, only_inputs=True
    )
    ...
    for i, parent_prod_spec in enumerate(parent_initial_producer_specs):
        ...
        if parent_data_name in handled_data:
            nested_initial_producer_specs.append(
                copy.copy(nested_initial_producer_specs[handled_data[parent_data_name]])
            )
            continue
        ...
        if len(repl_dict) != 0:
            raise NotImplementedError(
                "The renaming of `offset` and `subset` is not yet implemented."
            )
        ...
        top_state.add_edge(
            full_parent_priducer_specs[i].data_source[descening_point_spec].node,
            full_parent_priducer_specs[i].data_source[descening_point_spec].conn,
            descening_point_spec.consumer,
            nested_data_name,
            dace.Memlet(
                data=full_parent_priducer_specs[i].data_name,
                subset=dace_sbs.Range.from_array(full_parent_priducer_specs[i].desc),
            ),
        )
        assert nested_data_name not in descening_point_spec.in_connectors
        descening_point_spec.consumer.add_in_connector(nested_data_name, dtype=nested_desc.dtype)
        ...
        handled_data[parent_data_name] = len(nested_initial_producer_specs)
    ...
    # Remove the consumer
    dace_sdutils.remove_edge_and_dangling_path(top_state, descening_point_spec.edge)
    assert nested_concat_data not in descening_point_spec.consumer.out_connectors

    return nested_concat_data, nested_initial_producer_specs
