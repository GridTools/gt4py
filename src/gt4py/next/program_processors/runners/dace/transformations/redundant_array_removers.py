...
class DoubleWriteRemover(dace_transformation.SingleStateTransformation):
    ...
    def apply(
        ...
        # `parameter_mapping` maps dimensions of `temp_desc` to parameters of the Map.
        parameter_mapping = self._map_params_to_dimensions(
            inner_producer_subset, map_exit.map.params
        )

        # In case there are multiple consumers, we need an AccessNode on the inside of
        #  the Map that is used to distribute the data that is generated by the Map.
        #  Because of the checks in `can_be_applied()` we know that this is always a
        #  scalar. If there is already a scalar in the Map scope, we use that otherwise
        #  we generate a new one.
        if isinstance(inner_producer_edge.src, dace_nodes.AccessNode):
            inner_distribution_node: dace_nodes.AccessNode = inner_producer_edge.src
            assert isinstance(inner_distribution_node.desc(sdfg), dace_data.Scalar)
            assert not isinstance(inner_distribution_node.desc(sdfg), dace_data.View)
