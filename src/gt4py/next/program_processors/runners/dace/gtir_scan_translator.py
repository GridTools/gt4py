...
def _lower_lambda_to_nested_sdfg(
    ...
    Args:
        ...
        lambda_field_offsets: Mapping from symbol name to field origin,
            `None` if field origin is 0 in all dimensions.
    ...
    scan_loop_var = dace_utils.get_map_variable(scan_dim)
    ...
    scan_carry_input = (
        dace_utils.make_symbol_tree(scan_carry_symbol.id, scan_carry_symbol.type)
...
def translate_scan(
    ...
    if isinstance(scan_carry_type, ts.TupleType):
        lambda_flat_outs = {
            str(sym.id): sym.type
            for sym in dace_utils.flatten_tuple_fields(
