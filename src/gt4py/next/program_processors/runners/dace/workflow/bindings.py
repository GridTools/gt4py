...
def argument_processing_function(
    ...
    """Signature generated by `_create_binding_function()` and used to process the arguments for an SDFG call.

    See `_create_binding_function()` and `bind_sdfg()` for more.

    Returns:
        The function returns a `tuple` containing the processed positional arguments for
        the call and a `dict` containing the keyword arguments of the call.
...
def bind_sdfg(
    ...
    Refer to `_create_binding_function()` documentation.
    """
    ...
    bind_source = _create_binding_function(
