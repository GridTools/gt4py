...
def _parse_gt_param(
    ...
    if isinstance(param_type, ts.FieldType):
        if len(param_type.dims) == 0:
            ...
            if single_use_program:
                code.append(
                    f"assert {_cb_last_call_args}[{sdfg_arg_index}].value == {arg}.data_ptr()"
                )
            else:
...
def create_bindings(
    ...
    for i, param in enumerate(program_source.entry_point.parameters):
        ...
        _parse_gt_param(
            param.name, param.type_, arg, code, sdfg_arglist, sdfg_argnames, single_use_program
        )
    code.dedent()
...
def bind_sdfg(
    ...
    return stages.CompilableSource(
        program_source=inp, binding_source=create_bindings(inp, single_use_program)
    )
