...
def make_user_args(
    ...
    # If requested also add the metric arguments.
    if use_metrics:
        assert gtx_wfdcommon.SDFG_ARG_METRIC_LEVEL in sdfg.arglist()
...
class DaCeTranslator(
    ...
    def __call__(
        ...
        # NOTE: The bindings should actually be generated by `bind_sdfg()` however,
        #   there we do not have the necessarily arguments available to create them.
        binding_code = gtx_wfdbindings._create_sdfg_bindings(
            prog=program,
        ...
        #   of the SDFG and the bindings code on it.
        module: stages.ProgramSource[code_specs.SDFGCodeSpec] = stages.ProgramSource(
            ...
            source_code=(binding_code, gtx_wfdcommon.serialize_sdfg_as_json(sdfg)),  # type: ignore[arg-type]
            library_deps=tuple(),
