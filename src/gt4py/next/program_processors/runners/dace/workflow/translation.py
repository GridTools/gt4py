...
def make_user_args(
    ...
    """Generates the `user_args` field for `sdfg`.

    The generated signature is compatible with the one generated by the bindings
    function. See `_create_binding_function()` for more information.

    The metric arguments are detected by inspecting the SDFG's argument list.
    For that reason this function must be called _after_ they have been added.

    Args:
        program_parameters: The parameter list of the entry point function.
    ...
    def _make_user_args(
        ...
        if param_name.startswith("@"):
            ...
            return (
                (sdfg_connectivity_name, ("", "")) if sdfg_connectivity_name in sdfg_arglist else ""
            )

        elif isinstance(param_type, ts.TupleType):
            return tuple(
                _make_user_args(
                    sdfg=sdfg,
    ...
    if gtx_wfdcommon.SDFG_ARG_METRIC_LEVEL in sdfg_arglist:
        assert gtx_wfdcommon.SDFG_ARG_METRIC_COMPUTE_TIME in sdfg_arglist
        user_args.append(gtx_wfdcommon.SDFG_ARG_METRIC_LEVEL)
        user_args.append(gtx_wfdcommon.SDFG_ARG_METRIC_COMPUTE_TIME)
...
class DaCeTranslator(
    ...
    def __call__(
        ...
        program_parameters = self._make_entry_point_parameters(
            program, inp.args.offset_provider_type
        )

        # This function must be called _after_ the optimization of the SDFG has been finished.
        ...
        module: stages.ProgramSource[code_specs.SDFGCodeSpec] = stages.ProgramSource(
            ...
            source_code=gtx_wfdcommon.serialize_sdfg_as_json(sdfg),  # type: ignore[arg-type]
            library_deps=tuple(),
            code_spec=code_specs.SDFGCodeSpec(),
        )
        return module

    def _make_entry_point_parameters(
        self,
        program: itir.Program,
        offset_provider_type: common.OffsetProviderType,
    ) -> tuple[interface.Parameter, ...]:
        """Generates the parameter list for the entry point.

        The generated parameters are calling compatible with GTFN, this means:
        - Normal program arguments, i.e. `program.params`, come first.
        - Their name is unmodified, thus the name of argument `i` is `program.params[i].id`.
        - Tuple arguments are not flattened/expanded.
        - Followed by all `NeighborConnectivityType` offset providers, i.e. unused are
            not filtered out.
        - Offset providers are listed in the same order they appear in `offset_provider`.
        - The name of offset providers name is the string used as key in `offset_provider`
            prefixed by an `@`.
        """
        # Regular arguments
        assert all(param.type is not None for param in program.params)
        program_parameters: list[interface.Parameter] = [
            interface.Parameter(param.id, param.type)  # type: ignore[arg-type]
        ...
        return tuple(program_parameters)
