...
def convert_args(
    ...
    def decorated_program(
        *args: Any,
        ...
        # TODO(phimuell, edopao): Currently the `fast_call` API is not working. The main reason
        #   is that it updates only the non offset provider arguments. Which in most simulation
        #   might be enough. However, in the tests this might not be valid. For example consider
        #   a gird that is generated by a fixture, which does not has session scope.
        if True or (not fun.sdfg_program._lastargs):
