...
def _construct_if_branch_output(
    ...
    dims, origin, shape = gtir_domain.get_field_layout(
        gtir_domain.extract_domain(domain), ctx.domain_parser
    )
...
def translate_if(
    ...
    tbranch_ctx = gtir_to_sdfg.SubgraphContext(ctx.sdfg, true_state, ctx.domain_parser)
    ...
    fbranch_ctx = gtir_to_sdfg.SubgraphContext(ctx.sdfg, false_state, ctx.domain_parser)
