...
def get_map_variable(axis: gtx_common.Dimension | str) -> str:
    """
    Format map variable name based on the naming convention for application-specific SDFG transformations.
    """
    if isinstance(axis, gtx_common.Dimension):
        dim = axis
    elif len(axis) != 0:
        dim = gtx_common.Dimension(axis, gtx_common.DimensionKind.LOCAL)
    else:
