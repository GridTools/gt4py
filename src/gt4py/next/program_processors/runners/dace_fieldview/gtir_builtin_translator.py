...
class GtirBuiltinAsFieldOp(GtirTaskletCodegen):
    ...
    def __call__(self) -> list[tuple[dace.nodes.Node, ts.FieldType]]:
        ...
        field_name, _ = self._add_local_storage(self._field_type, field_shape)
        field_node = self._state.add_access(field_name)
        ...
        for arg, param in zip(self._args, self._stencil.params):
            ...
            if is_scalar:
                ...
                input_memlets[connector] = dace.Memlet(data=arg_node.data, subset=subset)
            else:
                input_memlets[connector] = dace.Memlet.from_array(
                    arg_node.data, arg_node.desc(self._sdfg)
                )

        # assume tasklet with single output
        output_index = ",".join(f"i_{dim.value}" for dim in self._field_type.dims)
        output_memlets = {output_connector: dace.Memlet(data=field_name, subset=output_index)}
        output_nodes = {field_name: field_node}
    ...
    def visit_SymRef(self, node: itir.SymRef) -> str:
        name = str(node.id)
        assert name in set(str(p.id) for p in self._stencil.params)
        return name
...
class GtirBuiltinSelect(GtirTaskletCodegen):
    def __init__(
        ...
        sdfg: dace.SDFG,
        state: dace.SDFGState,
        true_br_args: Sequence[GtirTaskletCodegen],
        false_br_args: Sequence[GtirTaskletCodegen],
    ):
        super().__init__(sdfg, state)
