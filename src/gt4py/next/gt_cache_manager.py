...
class BuildDir:
    ...
    def usable(self) -> bool:
        ...
        """
        return self.version_id == config.BUILD_CACHE_VERSION_ID
...
def get_translation_cache_dirs(
    ...
    return {
        backend: cache.get_translation_cache_folder(cache_base, backend) for backend in backends
    }
...
def find_build_dirs(cache_base: pathlib.Path, *, program: str | None = None) -> list[BuildDir]:
    ...
    for path in sorted(build_cache.iterdir()):
        ...
        if program is None or fnmatch.fnmatchcase(name, program):
            build_dirs.append(
                BuildDir(program=name, path=path, version_id=match.group("version_id"))
            )
    return build_dirs
...
def _cmd_path(args: argparse.Namespace) -> int:
    ...
    rows = [("lifetime", config.BUILD_CACHE_LIFETIME.name.lower()), ("cache base", str(cache_base))]
    rows += [
        (f"translation ({backend})", str(cache_dir))
        for backend, cache_dir in get_translation_cache_dirs(cache_base, args.backends).items()
    ]
    # Build folders are named per program variant, so name the pattern rather than a path.
    rows.append(
        (
            "build folders",
            (
...
def _list_by_program(args: argparse.Namespace, cache_base: pathlib.Path) -> int:
    ...
    if stale := sum(s.build_dirs - s.usable_build_dirs for s in summaries):
        print(
            f"\nnote: {stale} build folder(s) cannot be hit by this environment: they were"
...
def _add_common_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--backend",
        dest="backends",
        choices=[*cache.TRANSLATION_CACHE_BACKENDS, "all"],
        default="all",
...
def main(argv: Sequence[str] | None = None, *, prog: str | None = None) -> int:
    ...
    args.backends = (
        list(cache.TRANSLATION_CACHE_BACKENDS) if args.backends == "all" else [args.backends]
    )
    try:
