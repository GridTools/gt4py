...
source code specifications (`SourceCodeSpec` and friends), the source
containers produced and consumed by the pipeline steps (`ProgramSource`,
`BindingSource`, `ExtensionSource`) and the contracts of its end products
(`CompilationArtifact`, `ExecutableProgram`, `BuildSystemProject`).

Nothing in here knows about the GT4Py IRs or the DSL frontend; what a
program *is* stays in `gt4py.next.otf.stages`. The `next` type system is
not excluded: an entry point is an `otf.binding.interface.Function`, whose
parameters carry `TypeSpec`s, so the signature of a generated program stays
describable without reaching for an IR.
"""
