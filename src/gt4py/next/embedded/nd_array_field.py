...
class NdArrayField(
    ...
    def restrict(self, index: common.AnyIndexSpec) -> common.Field | core_defs.ScalarT:
        ...
        if new_domain.ndim == 0:
            ...
            index = tuple([index]) if not isinstance(index, tuple) else index  # type: ignore[assignment]
            if all([isinstance(i, core_defs.IntegralScalar) for i in index]):  # type: ignore[arg-type, union-attr]
