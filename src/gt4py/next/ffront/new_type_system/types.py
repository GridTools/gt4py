...
from gt4py.next.new_type_system import types as ts, traits, utils as ts_utils
...
class ScanOperatorType(ts.Type, traits.CallableTrait):
    ...
    def is_callable(self, args: Sequence[FunctionArgument]) -> traits.CallValidity:
        ...
        scalars = [ts_utils.unflatten_tuples(scalar, struct) for scalar, struct in zip(flat_scalars, structures)]
...
class BuiltinFunctionType(ts.Type, traits.CallableTrait):
    ...
    def is_callable(self, args: Sequence[FunctionArgument]) -> traits.CallValidity:
        ...
        return traits.CallValidity([f"callable trait for '{self.func.function.__name__}' not implemented"])
    ...
    def is_callable_broadcast(self, args: Sequence[FunctionArgument]) -> traits.CallValidity:
        ...
        if not isinstance(dims, ts.TupleType) or not all(isinstance(dim, DimensionType) for dim in dims.elements):
        ...
        if not all(dim in target_dims for dim in source_dims):
            return traits.CallValidity([f"broadcasting '{source}' to dimensions '{dims}' would remove dimensions"])
    ...
    def is_callable_as_type(self, args: Sequence[FunctionArgument]) -> traits.CallValidity:
        ...
        def can_cast_element(ty):
            ...
            if valid:
                return traits.CallValidity(FieldType(valid.result, ty.dimensions) if isinstance(ty, FieldType) else valid.result)
            return valid
    ...
    def is_callable_arithmetic_binop(self, args: Sequence[FunctionArgument]) -> traits.CallValidity:
        ...
        if not common_ty:
            return traits.CallValidity([f"could not promote '{lhs.ty}' and '{lhs.ty}' to a common arithmetic type"])
    ...
    def is_callable_where(self, args: Sequence[FunctionArgument]) -> traits.CallValidity:
        ...
        if not traits.is_implicitly_convertible(cond_elem, ts.BoolType()):
            return traits.CallValidity([(f"could not implicitly convert from '{cond_elem}' to '{ts.BoolType()}'"
                           f" (condition was '{cond.ty}')")])
        common_ty = traits.common_type(lhs_elem, rhs_elem)
        if not common_ty:
            return traits.CallValidity([(f"could not convert '{lhs_elem}' and '{lhs_elem}' to a common type"
