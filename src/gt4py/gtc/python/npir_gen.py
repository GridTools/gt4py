...
class NpirGen(TemplatedGenerator):
    ...
    DomainPadding = FormatTemplate(DOMAIN_PADDING_TPL)

    Computation = JinjaTemplate(COMPUTATION_JTPL)
    ...
    def visit_Computation(self, node, **kwargs):
        ...
        data_views = JinjaTemplate(DATA_VIEWS_JTPL).render(field_params=node.field_params)
        return self.generic_visit(
