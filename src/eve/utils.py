...
def get_typing(value: Any, *, get_origins: bool = True) -> Any:
    ...
    """
    recursive_get = functools.partial(get_typing, get_origins=get_origins)
    match value:
        case type():
            return value
        ...
        case tuple():
            ...
            return tuple[args[0], ...] if unique_type else tuple[args]
        case list() | set() | frozenset():
            t = type(value)
            unique_type, args = _collapsable_type_args(*(recursive_get(item) for item in value))
            return t[args[0]] if unique_type else t
        case dict():
            ...
            if unique_key_type or unique_value_type:
        ...
        case types.GenericAlias():
            args = tuple(recursive_get(a) for a in value.__args__)
            return value.__origin__[args]
        case t_value if t_value.__class__.__module__ == "typing":
            if get_origins:
                if hasattr(t_value, "__bound__"):  # TypeVar
                    return t_value.__bound__ or Any
                origin = typing.get_origin(t_value)
                args = tuple(recursive_get(a) for a in typing.get_args(t_value))
