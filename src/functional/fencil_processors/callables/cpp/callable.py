...
def create_callable(source_module: defs.SourceCodeModule, cache_strategy=CacheStrategy.SESSION) -> Callable:
    cache_folder = get_cache_folder(source_module.entry_point.name, source_module.source_code, cache_strategy)
    ...
    sources = {src_header_file: source_module.source_code,
               bindings_file: bindings_module.source_code}
    project = build.CMakeProject(name=source_module.entry_point.name, dependencies=deps, sources=sources)
