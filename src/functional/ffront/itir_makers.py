...
def ensure_expr(literal_or_expr: Union[str, int, itir.Expr]) -> itir.Expr:
    """
    Convert literals into a SymRef or NumberLiteral and let expressions pass unchanged.
    ...
    if isinstance(literal_or_expr, str):
        ...
        return itir.NumberLiteral(value=str(literal_or_expr), type="int")
...
class call_:
    ...
    --------
    >>> call_("plus")(1, 1)
    FunCall(fun=SymRef(id='plus'), args=[NumberLiteral(value='1', type='int'), NumberLiteral(value='1', type='int')])
    """
...
def shift_(offset, value=None):
    ...
    --------
    >>> shift_("i", 0)("a")
    FunCall(fun=FunCall(fun=SymRef(id='shift'), args=[OffsetLiteral(value='i'), NumberLiteral(value='0', type='int')]), args=[SymRef(id='a')])
...
def number_(type: str, value: str):
    return itir.NumberLiteral(value=value, type=type)
