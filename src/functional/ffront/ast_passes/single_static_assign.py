...
class SingleStaticAssignPass(ast.NodeTransformer):
    ...
    class RhsRenamer(ast.NodeTransformer):
        ...
        def __init__(self, state, separator):
            super().__init__()
            self.state = state
        ...
        def visit_Name(self, node: ast.Name) -> ast.Name:
            if node.id in self.state.name_counter:
                node.id = f"{node.id}{self.separator}{self.state.name_counter[node.id]}"
            return node
    ...
    def visit_Assign(self, node: ast.Assign) -> ast.Assign:
        ...
        node.value = self.RhsRenamer(self.state, self.separator).visit(node.value)
    ...
    def visit_Return(self, node: ast.Return) -> ast.Return:
        node.value = (
            self.RhsRenamer(self.state, self.separator).visit(node.value) if node.value else None
        )
        return node
