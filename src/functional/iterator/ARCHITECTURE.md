...
`fendef` return a wrapper that dispatches to a registered function. If `backend` is in the keyword arguments and not `None` `fendef_codegen` will be called and the result passed on to the backend via `execute_fencil()`, otherwise `fundef_embedded` will be called. The fendef wrapper also has a `.format()` method to invoke code generators and anything else that turns a fencil into text.
