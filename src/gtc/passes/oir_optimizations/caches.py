...
class IJCacheDetection(NodeTranslator):
    def visit_VerticalLoop(self, node: oir.VerticalLoop, **kwargs: Any) -> oir.VerticalLoop:
        ...
        cacheable = {
            ...
            and field not in {d.name for d in node.caches}
            and all(o[2] == 0 for o in offsets)
        }
        caches = self.visit(node.caches) + [oir.IJCache(name=field) for field in cacheable]
        return oir.VerticalLoop(
            interval=self.visit(node.interval),
            horizontal_executions=self.visit(node.horizontal_executions),
            loop_order=self.visit(node.loop_order),
